...
| Remote action schemas | [`src/Schema/RemoteSchemaCatalog.pas`](/home/ubuntu/MethylPipeline/tools/methyl-config-editor/src/Schema/RemoteSchemaCatalog.pas) — `GET /v1/actions` when `[Gateway] Enabled=true` |
...
| **`IGatewayService` / `TGatewayService`** | One method per OpenAPI operation; calls `sp_*` or repository procs |
