...
| methylGrapher WGBS align | `methylgrapher.wgbs_align` | External Docker (**CPU**; no CUDA) | QC BAM + `{id}.alignment.gaf` + metrics |
...
| methylGrapher WGBS extract | `methylgrapher.wgbs_extract` | External Docker (**CPU**; no CUDA) | Graph-aware H5 + patterns + canonical manifest |
