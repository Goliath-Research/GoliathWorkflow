...
Three alignment modes (linear / stock Giraffe / methylGrapher WGBS) share the same QC gates; extract branches on `useWgbsPangenome`. Source: [`sample_prep.program.json`](/home/ubuntu/MethylPipeline/workflow_engine/domain/fixtures/sample_prep.program.json).
...
flowchart TD
  subgraph perSample [FOREACH sample parallel]
    dl[download_FASTQs]
    mode{align_mode}
    fq[parabricks_fq2bam]
    gf[parabricks_giraffe]
    mg[methylgrapher_wgbs_align]
    qc[methyl_qc alignment guardrail]
    gateQc{qcPass}
    frag[methyl_fragmentomics if isCfdna]
    extMode{useWgbsPangenome?}
    mex[methyl_extract]
    mgExt[methylgrapher_wgbs_extract]
    extQc[extraction_qc post-extract guardrail]
    gateExt{extractionQcPass}
    archive[archive_sample optional]
    delFq[delete_fastqs if deleteFastqs]
    delBam[delete_BAM]
    trim[trim_fastq fastp]
    align2[same align mode forceRealign]
    qc2[methyl_qc retry]
    fail[sample_qc_failed]
  end
  dl --> mode
  mode -->|linear| fq --> qc
  mode -->|pangenome| gf --> qc
  mode -->|pangenome_wgbs| mg --> qc
  qc --> gateQc
  gateQc -->|pass| frag --> extMode
  extMode -->|no| mex --> extQc
  extMode -->|yes| mgExt --> extQc
  extQc --> gateExt
  gateExt -->|pass| archive --> delFq --> delBam
  gateExt -->|fail| fail
  gateQc -->|fail + remediate| trim --> align2 --> qc2 --> gateQc
  gateQc -->|fail terminal| fail
...
| Parabricks Giraffe | `parabricks.giraffe` | External GPU | Stock pangenome surjected BAM + QC tar |
| methylGrapher WGBS align | `methylgrapher.wgbs_align` / `wgbs_gpu_align` | External Docker (**native-Mojo GPU**; NVIDIA CUDA or AMD HIP) | QC BAM + `{id}.alignment.gaf` + `alignment_metrics.json` |
| methyl-qc | `methyl-qc` | In-repo (mode-aware) | V2 QC JSON; `qcPass`, remediation scope vars |
| Trim FASTQ | `sample.trim-fastq` | In-process fastp | Trimmed FASTQs (remediation branch) |
| Delete FASTQs | `sample.delete-fastqs` | In-process | FASTQs removed after final QC (when `deleteFastqs`) |
...
| methylGrapher WGBS extract | `methylgrapher.wgbs_extract` | External Docker (native-Mojo MethylCall/MergeCpG; CPU-parallel) | Graph-aware H5 + patterns + canonical manifest |
| Extraction QC | `methyl-extraction-qc` | In-repo | `{id}.extraction_qc.json`; `extractionQcPass` |
| Archive sample | `sample.archive-sample` | In-process | Optional remote qc/fastq/h5 bundle (`mode=full` or `qc_only`) |
| Delete BAM | `sample.delete-bam` | In-process | BAM removed after extraction QC disposition |

**Ordering:** Run **methyl-qc before methyl-fragmentomics** so failed samples do not scan BAMs. **extraction_qc** runs after extract and before archive/delete. FASTQs are retained until final QC so trim/realign can run on the **same** alignment mode. Read-level `{chr}-CG.patterns.h5` sidecars are emitted at extract time; **`pipeline.info_measures` runs later in the study lifecycle**, not inside SamplePrep. When `primary_analyte` is `cfdna`, fragmentomics runs; `buffy_coat` skips it.

### Storage contract (`/work/samples`)

```
/work/samples/{sample_id}/
  *.fastq.gz              (transient; deleted after archive when deleteFastqs)
  {sample_id}.bam         (transient; deleted after extract disposition)
...
**Schema boundary:** Domain configuration (archive storage profiles, portal RBAC, cohort metadata) belongs in **`portal`**, **`Meta`**, **`RBAC`**, or **`cfg`** schemas — not in **`wf`**. The engine stores opaque `context_json` with already-resolved action inputs (e.g. `fastqStorage`, `sampleStorage` / `h5Storage`); middle-tier planners read `portal.resource_profile` (which may resolve a named `cfg.storage_endpoint`) before instance creation. See [`docs/deployment/portal_resource_profile.md`](/home/ubuntu/MethylPipeline/docs/deployment/portal_resource_profile.md).
