...
BEGIN
    ...
    /*
      Azure SQL forbids implicit json ↔ nvarchar. The proc contract is json;
      live wf.scope_variable.value_json may still be nvarchar(max) until
      wf_json_column_alignment.sql. Dynamic SQL picks the matching assignment
      so CREATE/EXEC succeed in either state.
    */
