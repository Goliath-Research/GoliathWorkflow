# methylGrapher-mojo dual-ship image (engine + optional Mojo CLI) for WGBS SamplePrep.
# Same vg/jemalloc=off story as Dockerfile; entrypoint stays `methylGrapher`.
#
# Build via scripts/build_methylgrapher_mojo_image.sh (stages engine/ from
# METHYLGRAPHER_MOJO_ROOT into this directory before docker build).
#
# Smoke:
...
# Patched engine (GAF header skip + single GFA worker) — primary entrypoint.
COPY engine /opt/methylgrapher-mojo/engine
COPY methylGrapher.mojo.sh /usr/local/bin/methylGrapher
RUN chmod +x /usr/local/bin/methylGrapher \
...
LABEL org.opencontainers.image.title="methylGrapher-mojo WGBS worker" \
      org.opencontainers.image.description="methylGrapher-mojo engine + vg for pangenome_wgbs SamplePrep" \
