#!/usr/bin/env bash
# Run Monte Carlo gene stability for Buffy-coat on a VM with /work NFS mounted.
# Pair with scripts/run_plasma_mc_sweep.sh on a second VM, then compare:
#   scripts/compare_analyte_sweep_results.sh --sweep-id <ID>
#
# Example:
#   scripts/run_buffy_mc_sweep.sh \
...
Buffy-coat MC stability runner for shared /work storage.
Writes outputs under:
  /work/projects/prostate-cancer/sweeps/<sweep-id>/[variant/]buffy/
  .../Buffy_healthy_vs_PCa/monte_carlo_runs/

$(mc_sweep_usage_header)

Buffy-specific:
  BUFFY_PROJECT          Study manifest (default: project_Buffy_healthy_vs_PCa.json)
EOF
}
...
if [[ ! -f "$PROJECT_JSON" ]]; then
  echo "Error: buffy project manifest not found: $PROJECT_JSON" >&2
  exit 1
fi
...
echo "Buffy sweep complete: sweep_id=$SWEEP_ID output_base=$OUTPUT_BASE"
echo "When Plasma finishes on the other VM, run:"
