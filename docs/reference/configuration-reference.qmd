...
**Note.** Do not repeat the `controls`/`diseases` block inside `actionConfig.predictor`. Sample paths are inherited from the project's top-level cohort structure. See @sec-project-configuration for the correct anti-pattern avoidance.
...
|---|---|---|
| `actionConfig.validation.run_mapper_and_enricher` | Legacy behavior flag | Deprecate, then remove. Freeze stage owns mapper/enricher execution. |
| `actionConfig.validation.skip_enricher` | Legacy companion flag | Deprecate, then remove. Use explicit freeze/enricher step policy instead. |
...
#### 2) Remove `run_mapper_and_enricher` / `skip_enricher` from `actionConfig.validation`
