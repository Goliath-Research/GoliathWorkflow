...
Named staged/MC packs (repo): `staged_ovr_mc`, `staged_full_lifecycle`, `staged_progression_interpretation`, and the SaMD ladder `samd_research` → `samd_holdout_enrichment` → `samd_pivotal` (see [`docs/usage/18-samd-study-lifecycle.md`](../usage/18-samd-study-lifecycle.md)). Research statistical axes are **`samd_research` + `researchMode`** overlays (`dmp_raw`, `dmp_fc`, `gene_enricher`, `gene_fc`, `dual_fc` under `workflow_engine/domain/profiles/modes/`). Legacy `mc_dmp`, `mc_dmp_fc`, `mc_gene`, `mc_gene_fc`, `mc_dmp_gene_fc` (and older aliases such as `buffy_mc_gene_fc`) remain deprecated name aliases that fold into those modes. See [`workflow_engine/domain/profiles/`](../../workflow_engine/domain/profiles/).
