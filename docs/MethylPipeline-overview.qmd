...
\begin{tikzpicture}[
  ...
  node distance=0.78cm and 0.55cm,
  base/.style={draw, rounded corners=3pt, align=center, font=\footnotesize, minimum height=0.78cm, text width=2.55cm},
...
\node[core, above left=1.2cm and 1.45cm of utils] (centroid) {methylcentroid\\cohort centroids};
\node[core, above=1.4cm of utils] (detector) {methyldetector\\DMP discovery};
\node[core, above right=1.2cm and 1.45cm of utils] (classifier) {methylclassifier\\classification};
\node[core, right=2.35cm of classifier] (predictor) {methylpredictor\\prediction metrics};
\node[core, left=2.35cm of centroid] (validation) {methylvalidation\\stability, freeze, model build};

\node[support, below=1.5cm of utils] (mapper) {methylmapper\\gene and region mapping};
\node[support, below right=1.25cm and 1.45cm of utils] (enricher) {methylenricher\\enrichment and modules};
\node[support, right=2.35cm of enricher] (qc) {methylalignmentqc\\alignment QC parsing};
...
\draw[arr] (qc) to[out=170,in=-35] (validation);
\end{tikzpicture}
