...
**Operator reference:** [`workflow_engine/sql/SamplePrepFlow.md`](../../workflow_engine/sql/SamplePrepFlow.md)
...
\begin{tikzpicture}[
  ...
  box/.style={draw, rounded corners, align=center, minimum width=3.4cm, minimum height=0.75cm, font=\small}
]
\node[box] (dl) {download FASTQs};
\node[box, below=of dl] (align) {Parabricks fq2bam};
\node[box, below=of align] (aqc) {methyl-qc\\(alignment guardrail)};
\node[box, below=of aqc] (gate1) {qcPass?};
\node[box, below left=1.0cm and 0.6cm of gate1] (rem) {trim + realign\\(if fixable)};
\node[box, below right=1.0cm and 0.6cm of gate1] (frag) {fragmentomics\\(cfDNA only)};
\node[box, below=of frag] (ext) {MethylExtractor};
\node[box, below=of ext] (eqc) {methyl-extraction-qc};
\node[box, below=of eqc] (gate2) {extractionQcPass?};
\node[box, below=of gate2] (h5) {archive H5 + cleanup};

\draw[->] (dl) -- (align) -- (aqc) -- (gate1);
\draw[->] (gate1) -- node[right, font=\scriptsize] {fail + fixable} (rem);
\draw[->] (rem) -- (aqc);
\draw[->] (gate1) -- node[right, font=\scriptsize] {pass} (frag);
\draw[->] (frag) -- (ext) -- (eqc) -- (gate2) -- (h5);
\end{tikzpicture}
\end{center}
```
...
These feed `summary_stats` on the export JSON (total reads, duplication rate, etc.). Optional config guardrails (`step_config.alignment_qc.optional_guardrails.duplication_rate_max`) can fail samples with excessive PCR duplication even when cycle quality looks fine.
...
Configure under `step_config.alignment_qc` in the project JSON:
...
python scripts/alignment_qc_cohort_screening.py \
  --qc-dir /work/prostate-cancer/alignment_qc \
  --group healthy=/work/prostate-cancer/data/healthy.csv \
  --group pca=/work/prostate-cancer/data/pca.csv \
  --out /work/prostate-cancer/alignment_qc/screening_report
```
...
Override thresholds via `step_config.extraction_qc` in the project JSON. Schema: `schemas/extraction_qc_output.schema.json`.
...
methyl-qc --samples /work/samples/SAMPLE_ID --output-dir /work/prostate-cancer/alignment_qc

# Project-scoped (uses step_config.alignment_qc)
methyl-qc --project /work/prostate-cancer/configs/project_Example.json
...
- Workflow operator guide: [`workflow_engine/sql/SamplePrepFlow.md`](../../workflow_engine/sql/SamplePrepFlow.md)
