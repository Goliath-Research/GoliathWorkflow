...
\node[box] (portal) {Portal / SQL procs};
\node[box, right=of portal] (db) {Database wf schema};
\node[box, right=of db] (gw) {methyl-gateway};
\node[box, right=of gw] (worker) {methyl-worker GPU};
\node[box, below=1.0cm of worker] (work) {Shared /work storage};
...
4. [ ] Seed action catalog: `methyl-export-task-schemas && methyl-export-action-catalog && python workflow_engine/sql/seed_action_catalog.py`
...
| Bundled schema | `workflow_engine/sql/MethylPipeline.sql` |
...
# Deploy SamplePrep + StudyValidation DomainPrograms to gateway:
export GATEWAY_URL=https://your-gateway.example.com
export GATEWAY_ADMIN_BEARER_TOKEN='...'   # when Entra required
bash scripts/deploy_workflow_definitions.sh
```

Output: `workflow_versions.json` with `workflow_version_id` values for API calls.
...
Authoring language: [`docs/domain_program_language.md`](../../domain_program_language.md)
...
- `GATEWAY_REQUIRE_ENTRA` — require JWT on admin/operator routes
- `GATEWAY_ENTRA_AUDIENCE`, `AZURE_TENANT_ID` — Entra validation
...
{
  "projectPath": "/work/epimethyl/data/project_Example.json",
  "workflow_version_id": <from workflow_versions.json>,
  "fastqStorage": { "type": "s3", "bucket": "...", "region": "us-west-2" },
  "sampleCsvs": ["/work/.../samples.csv"]
}
```
...
Or staged CLI: [Stage: Stability](05-stage-stability.qmd).
...
- SamplePrep operator guide: [`workflow_engine/sql/SamplePrepFlow.md`](../../workflow_engine/sql/SamplePrepFlow.md)
