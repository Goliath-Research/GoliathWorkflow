...
# or:  methyl-validation plan-runs --project /work/prostate-cancer/configs/project_Healthy_vs_PCa1-4-CG.json --overwrite
```

`--overwrite` only replaces `queue/tasks/` and regenerates the plan; it does **not** remove existing `run_####` output directories. To delete all `run_####` trees before planning, add `--wipe-runs` (irreversible).
...
```bash
methyl-validation run-task --task /work/prostate-cancer/Healthy_vs_PCa1-4-CG/monte_carlo_runs/queue/tasks/run_0001.json
```
...
- [Optional hyperparameter search](13-optional-hyperparameter-search.qmd) (search trials can use the same shared storage)
