...
|-----------|---------|----------|
| `buffy_wgbs_pangenome_gene_fc` | `buffy_coat` | **Default** WGBS pangenome (methylGrapher, `alignmentMode: pangenome_wgbs`); requires `hprc-d9-bs.wl.gfa` |
| `buffy_wgbs_linear_gene_fc` | `buffy_coat` | Linear WGBS baseline (same science knobs, cheaper aligner) |
...
Use `buffy_wgbs_linear_gene_fc` when GPU-only / lower wall-time is preferred.
