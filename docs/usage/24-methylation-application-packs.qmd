...
|-----------|---------|----------|
| `buffy_wgbs_pangenome_gene_fc` | `buffy_coat` | Pangenome (Giraffe), read-level informME, Houseman deconv, gene FeatureCuts, full lifecycle |
| `buffy_wgbs_linear_gene_fc` | `buffy_coat` | Linear WGBS baseline (same science, cheaper aligner) |
...
8. Add CI smoke fixture + pack regression test (mirror Alzheimer / plant tests).
9. Write a short usage chapter (instance-only deltas) and add a regulatory roadmap row.
10. Validate: `methyl-study-validate-manifest --profile samd_research` (or higher ladder tier).
