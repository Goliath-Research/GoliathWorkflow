...
5. Confirm analyte + site: human GRCh38 vs plant/other linear genome pins (Arabidopsis
   TAIR10, soybean Wm82, maize B73, wheat IWGSC recipes); omit blood deconvolution when
   bases do not apply (or enable plant atlases via `*_basis_path` + the with-deconv
   lifecycle). For plant trait priors use `enrich_source: plant_traits` + an offline TSV —
   not Open Targets. For epi-GBS use `libraryProtocol: epi_gbs` +
   `sample_prep_epigbs.program.json` (not Parabricks WGBS SamplePrep).
