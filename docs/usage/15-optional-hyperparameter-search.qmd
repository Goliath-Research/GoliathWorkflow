...
**Scenario (assumption check):** `methyl-study-start scenario-start` with a `HyperparamScenarioRequest` overlay — one workflow instance + `executionScopeId`, no Cartesian axes. Use this to confirm personal assumptions or evaluate a stability target (BA gates, FeatureCuts caps, frequencies). JSON `null` clears a site/profile knob.
