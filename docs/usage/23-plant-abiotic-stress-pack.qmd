...
extends that pattern with plant **platform unblockers** because human blood/cfDNA
defaults are biologically wrong for plant WGBS: a `plant_tissue` analyte, a non-human
site reference, a lifecycle program without blood cell deconvolution, and a plant
enrichment preset. It reuses SamplePrep, the centroid → detector → mapper → Monte Carlo
stability science, and `samd_research`. There is no new aligner or workflow action.
