...
| Goal | `pipelineProfile` |
|------|-------------------|
| Standard binary Monte-Carlo stability (DMP + gene FeatureCuts) | `mc_dmp_gene_fc` |
...
### Statistical modeling modes (`mc_*`)

Five reusable **research axes** for Monte-Carlo stability. See also [Stage: Stability](05-stage-stability.qmd).

| Mode | Profile | `dmp_modeling_mode` | `gene_modeling_mode` | Use when |
|------|---------|---------------------|----------------------|----------|
...
| 3 | `mc_gene` | `raw_pool` | `none` | Enricher gene recurrence (PPI hubs or Enrichr) |
...
: Statistical modeling modes {tbl-colwidths="[6,14,18,18,44]"}

`mc_gene` and `mc_gene_fc` default to `actionConfig.enricher.ppi_only: true` (STRING PPI hubs). Set `ppi_only: false` in a site/profile/instance override for Enrichr pathway libraries.
...
|---------|---------------------|------------|----------------------|-------------|
| `mc_dmp` | discovery only | `dmps-*-discovery.csv` | DMP recurrence (raw pool) | DMP-axis research |
| `mc_dmp_fc` | discovery + `dmp_select` | `dmps-*-selected.csv` | DMP panel MC + FeatureCuts | BA-gated DMP panel |
| `mc_gene` | discovery only | discovery | enricher gene recurrence | Gene-axis research (PPI/Enrichr) |
| `mc_gene_fc` | discovery only | discovery | gene FeatureCuts on enricher genes | Gene-axis BA gate |
| `mc_dmp_gene_fc` | discovery + `dmp_select` | discovery / selected | DMP + gene FeatureCuts MC | **Production** binary stability |
...
**Deprecated aliases** (resolve to canonical `mc_*` files in `pipeline_profiles.py`): `mc_dmp_discovery` → `mc_dmp`, `mc_dmp_featurecuts` → `mc_dmp_fc`, `mc_gene_mapper` → `mc_gene`, `mc_gene_featurecuts` → `mc_gene_fc`, `discovery_gene_featurecuts` → `mc_dmp_gene_fc`, `dmp_panel_stability` → `mc_dmp_fc`, `gene_enricher_stability` → `mc_dmp`, `buffy_mc_gene_fc` → `mc_dmp_gene_fc`.
