...
The profile supplies the science knobs (iterations, thresholds, feature caps). Pick by what you want out of the run:
...
| Progression / interpretation only | `staged_progression_interpretation` |

All profiles live in `workflow_engine/domain/profiles/`. See the profile/mode table in [Stage: Stability](05-stage-stability.qmd).
...
3. Pick a profile (`mc_dmp_gene_fc` binary dual-axis, `mc_dmp` / `mc_dmp_fc` / `mc_gene` / `mc_gene_fc` for research axes, `staged_ovr_mc` multi-stage).
