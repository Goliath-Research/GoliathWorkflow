...
- WGBS methylGrapher: `alignmentMode: "pangenome_wgbs"` (binds `useWgbsPangenome` + `usePangenome`). Procedure `buffy_wgbs_pangenome_gene_fc` selects this mode (**experimental** until acceptance).
- **Interim buffy recommendation:** use procedure `buffy_wgbs_linear_gene_fc` (`alignmentMode: linear`). Do not promote `pangenome_wgbs` as the production buffy default until the linear-vs-wgbs acceptance gate passes (see [`docs/plans/wgbs-alignment-decision.plan.md`](../plans/wgbs-alignment-decision.plan.md)).
