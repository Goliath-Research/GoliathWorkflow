...
WGBS pangenome align writes **`{sample_id}.alignment_metrics.json`** (tool=`methylGrapher`) plus GAF / QC BAM / dedup. Export may include optional `wgbs_align_metrics`; Parabricks-only payload blocks stay `null`. Guardrails live in `wgbs_pangenome_qc.py`.
