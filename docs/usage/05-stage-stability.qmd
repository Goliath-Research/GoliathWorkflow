...
|------|---------|-----------|
| DMP recurrence only | `mc_dmp_discovery` | `dmp_modeling_mode: raw_pool` |
| BA-gated DMP panel | `mc_dmp_featurecuts` | `dmp_modeling_mode: featurecuts` |
| Mapper gene recurrence | `mc_gene_mapper` | `gene_modeling_mode: mapper_ranked` |
| Gene FeatureCuts on discovery loci | `mc_gene_featurecuts` | `gene_modeling_mode: featurecuts` |
| Full DMP + gene FC (production) | `mc_gene_fc` | both axes featurecuts |
...
Deprecated names (`discovery_gene_featurecuts`, `dmp_panel_stability`, `gene_enricher_stability`, `buffy_mc_gene_fc`) alias to the generic presets in `pipeline_profiles.py`.
