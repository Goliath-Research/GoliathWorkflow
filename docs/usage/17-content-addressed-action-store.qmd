...
- **Site / worker:** `METHYL_CAAS_ENABLED=1` in worker environment.
- **Per instance / task:** `"caasEnabled": true` in instance context or task input.

Leave disabled during initial debugging if you need to inspect plain files without symlinks.
...
`finalize_instance_context` bakes `hyperparamSetId` from all merged `resolvedConfig__*` slices (optional label via `hyperparamSetName`). Every ACTION template receives `hyperparamSetId`; workers stamp it on manifests and append ledger entries.

Use the ledger to compare which content keys each hyperparameter set resolved to (for example Balanced Accuracy per set).

## Database mirror (distributed gateway)

When the gateway and workers run against PostgreSQL or Azure SQL:

- `wf.hyperparameter_set` — set identity and merged config snapshot
- `wf.workflow_instance.hyperparameter_set_id` — links instance to set
- `wf.hyperparameter_set_action_entry` — action/run_key → content_key

Deploy: `workflow_engine/sql_pg/wf_hyperparameter_set.sql` (parity in `sql_mssql/`). Instance creation calls `wf_apply_hyperparameter_set`; successful task submits upsert action entries when CAAS is enabled.
...
- [ ] `METHYL_CAAS_ENABLED=1` (or `caasEnabled: true`) on workers that should version results
