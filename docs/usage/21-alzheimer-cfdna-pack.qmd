---
title: "Alzheimer cfDNA disease pack"
---

Alzheimer detection ships as a **disease pack** on the existing DNA-methylation process,
not a new omics modality. It reuses the SamplePrep and study-lifecycle DomainPrograms,
the `samd_*` profile ladder, and the cfDNA analyte profile; the disease-specific parts
are cohorts, patient-disjoint partitions, and a small config overlay. There is no new
aligner, action, or program.
...
```mermaid
flowchart TD
  H["healthy.csv (Control)"] --> CMP{"comparisons<br/>control_vs_each_disease"}
  MCI["mci.csv (AD_MCI)"] --> CMP
  AD["ad.csv (AD_AD)"] --> CMP
  CMP --> MC["MC stability per comparison"]
  MC --> PROG["pipeline.progression<br/>progression_labels: AD_MCI, AD_AD"]
