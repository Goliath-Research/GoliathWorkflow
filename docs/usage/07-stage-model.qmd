...
For ECDF with leukocyte covariates, the second-stage LR uses six independent
features: the clipped log-odds of `prob_class1` and five standardized additive
log-ratios (`CD8T`, `CD4T`, `NK`, `Bcell`, `Mono`) relative to `Neu`. The exact
matrices are saved under each run's `model_bundle/second_stage/`; its manifest
records transform parameters and verifies zero train/test sample overlap.
