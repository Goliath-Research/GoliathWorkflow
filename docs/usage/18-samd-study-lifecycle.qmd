...
Add `--modality rnaseq` for an RNA-Seq study (see [ch.20](20-rnaseq-process-pack.qmd)); the default is `methylation`. For a worked disease pack on the methylation modality, see the [Alzheimer cfDNA pack](21-alzheimer-cfdna-pack.qmd) (staged Control -> MCI -> AD, cfDNA, `neuro-core` enrichment). For a non-human trait pack on the same modality, see the [Plant abiotic stress pack](23-plant-abiotic-stress-pack.qmd) (Arabidopsis Control vs Drought, `plant_tissue` analyte, CG/CHG/CHH).
