...
For a first binary or multi-stage walkthrough without the SaMD ladder, use [Tutorial ch.16](16-tutorial-healthy-vs-cancer-stages.qmd). For hyperparameter search mechanics, see [ch.15](15-hyperparameter-search.qmd) and [`packages/methylvalidation/docs/HYPERPARAMETER_SEARCH.md`](../../packages/methylvalidation/docs/HYPERPARAMETER_SEARCH.md).
...
![SaMD study lifecycle](../diagrams/out/samd-study-lifecycle.png)
...
Add `--modality rnaseq` for an RNA-Seq study (see [ch.20](20-rnaseq-process-pack.qmd)); the default is `methylation`. Config overlays on an existing process are **application packs** — see the generic pattern in [ch.24](24-methylation-application-packs.qmd). Worked methylation instances: [Alzheimer cfDNA](21-alzheimer-cfdna-pack.qmd) (disease — staged Control -> MCI -> AD, cfDNA, `neuro-core`) and [Plant abiotic stress](23-plant-abiotic-stress-pack.qmd) (trait — Arabidopsis Control vs Drought, `plant_tissue`, CG/CHG/CHH).
