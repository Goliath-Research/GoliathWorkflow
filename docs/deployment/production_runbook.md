...
**Alignment modes:** each sample uses one path from instance/profile `alignmentMode` — `linear` (explicit Clara **or** MojoFq2bamMeth via `actionConfig.parabricks.engine`), `pangenome` (stock Giraffe via Clara — explicit), or `pangenome_wgbs` (**native-Mojo** Giraffe dual C2T/G2A on NVIDIA CUDA or AMD HIP via `sample.methylgrapher_wgbs_align` / `_extract`; `giraffe_device=auto|nvidia|amd`; CPU only for unknown GPU vendors). **Buffy default:** procedure `buffy_wgbs_pangenome_gene_fc`. Linear Clara: `buffy_wgbs_linear_gene_fc`; linear Mojo: `buffy_wgbs_linear_mojo_gene_fc`. Clara is never an automatic Mojo failure path. See [`mojo-multi-gpu-dual-align.md`](../architecture/mojo-multi-gpu-dual-align.md). Program checks `useWgbsPangenome` **before** `usePangenome` — do not fall back to stock Giraffe when the BS bundle is missing.

**WGBS pangenome operator checklist:**

- [ ] Site `pangenome_wgbs_genome` / `actionConfig.methylgrapher_wgbs` provisioned (`pangenome-grch38-d9-bs-1.70` on QNAP → `/work/genomes/…/d9-bs/1.70`), including `hprc-d9-bs.wl.gfa` for MethylCall
- [ ] `METHYL_METHYLGRAPHER_IMAGE` / site `actionConfig.methylgrapher_wgbs.image` pinned to `:1.70-mojo-cuda` (NVIDIA) or `:1.70-mojo-rocm` (AMD); `engine=mojo`, `align_engine=gpu_giraffe|mojo_giraffe` (64K-safe jemalloc=off vg in image on Grace/GH200; see [`workers/docker/methylgrapher/README.md`](../../workers/docker/methylgrapher/README.md)).
- [ ] Acceptance thresholds available at `tests/real_data/sample_prep_mode_compare/thresholds.acceptance.json` for re-qualification
...
| Switch methylGrapher engine | Site `actionConfig.methylgrapher_wgbs.engine` = `mojo` + `image` `:1.70-mojo-cuda` or `:1.70-mojo-rocm` + `align_engine=gpu_giraffe|mojo_giraffe` (native-Mojo Align/MethylCall/MergeCpG). Dev/parity: `python` + `:1.70`. Unknown GPU vendor: `cpu_vg`. Do **not** auto-fall back to Clara — change `alignmentMode` explicitly for linear/stock Parabricks. Publish images to ACR / shared tar; rebuild via `scripts/build_methylgrapher_mojo_image.sh`. See [`docs/plans/methylgrapher-mojo-cutover-gate.md`](../plans/methylgrapher-mojo-cutover-gate.md). |
| GH200 dual-graph Align / Mojo Giraffe | Site `align_engine`: `cpu_vg`, `mojo_giraffe`, or `gpu_giraffe`. `gpu_giraffe` prefers **GBZ-native Mojo** (PrepareGenome quartet; no GFA size-cap). Build strand caches with `build_mojo_gbz_cache.py` before full Buffy. Emergency: `FALLBACK=vg`. Linear/`fq2bam_meth` is comparator only for Buffy WGBS. Do **not** claim “map done” until Buffy ≤~2 h + MethylCall parity ([`gbz-native-mojo-giraffe.plan.md`](../plans/gbz-native-mojo-giraffe.plan.md), [`mojo-giraffe-cutover-gate.md`](../plans/mojo-giraffe-cutover-gate.md)). |
| Full Buffy 238 re-extract | **Blocked** until missing25 SamplePrep COMPLETED (gateway instance started for the 25-sample context). Then open a dedicated ops/plan run — do not mix with in-flight Align. |
| WGBS Picard QC tables | Align may soft-fail `collectmultiplemetrics` on the restored QC BAM; mode-aware QC enriches only when provenance has `collectmultiplemetrics: true`. |
