...
**Alignment modes:** each sample uses one path from instance/profile `alignmentMode` — `linear` (`parabricks.fq2bam`: Clara **or** MojoFq2bamMeth via `actionConfig.parabricks.engine`), `pangenome` (stock Giraffe via Parabricks), or `pangenome_wgbs` (Mojo Giraffe dual C2T/G2A via `sample.methylgrapher_wgbs_align` / `_extract`; portable `align_device=auto|nvidia|amd|cpu`). **Buffy default:** procedure `buffy_wgbs_pangenome_gene_fc`. Linear Clara: `buffy_wgbs_linear_gene_fc`; linear Mojo: `buffy_wgbs_linear_mojo_gene_fc`. See [`mojo-multi-gpu-dual-align.md`](../architecture/mojo-multi-gpu-dual-align.md). Program checks `useWgbsPangenome` **before** `usePangenome` — do not fall back to stock Giraffe when the BS bundle is missing.
...
| Switch methylGrapher engine | Site `actionConfig.methylgrapher_wgbs.engine` = `mojo` + `image` `:1.70-mojo` (native MethylCall/MergeCpG/Align; MethylCall `-t` uncapped) or `python` + `:1.70` one-release rollback. Publish image to ACR `epicontainerregistry1.azurecr.io/epimethyl/methylgrapher:1.70-mojo` and/or shared tar `/work/epimethyl/docker/images/methylgrapher-1.70-mojo.aarch64.tar.gz`. Worker pin: `METHYL_METHYLGRAPHER_IMAGE` + optional `METHYLGRAPHER_ALIGN_ENGINE=gpu_giraffe`. In-image emergency: `METHYLGRAPHER_MCALL_ENGINE=python`. Rebuild via `scripts/build_methylgrapher_mojo_image.sh`. See [`docs/plans/methylgrapher-mojo-cutover-gate.md`](../plans/methylgrapher-mojo-cutover-gate.md). |
