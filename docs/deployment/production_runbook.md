...
| GH200 dual-graph Align | Site `align_engine`: `cpu_vg` (default) or `gpu_giraffe`. Parabricks 4.7 cannot emit science GAF — Phase 0 NO-GO; `gpu_giraffe` uses vg GAF fallback (`METHYLGRAPHER_GPU_GIRAFFE_FALLBACK=vg`). Do **not** flip GH200 fleets to `gpu_giraffe` as “GPU map done” until full-sample ≤~2 h gate. Workers with NVIDIA GPU advertise capability `methylgrapher.wgbs_gpu_align`. See [`docs/plans/gh200-wgbs-dual-graph-align.plan.md`](../plans/gh200-wgbs-dual-graph-align.plan.md). |
