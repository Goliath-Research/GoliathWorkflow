...
`--join-mode auto` (default): if `current/manifest.json` is missing, this VM seeds shared `/work` then enrolls; if present, it installs VM-local Docker/CTK/host tools (skip Parabricks pull) then enrolls so capability probe sees host CLIs.
