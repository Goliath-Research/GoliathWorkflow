...
- `methylcluster` uses beta-based likelihoods and distance geometry,
...
Several code paths look incomplete or partially superseded:

- `packages/methylutils/comparison.py` still advertises beta-based comparison logic that does not match the active ECDF-first production path.
- `packages/methylcluster/methyl_cluster/methyl_cluster_dp.py` contains a Dirichlet-process-like branch with an unimplemented prior component.
- Some older package docs still describe a stricter or cleaner theory story than the code currently supports.

These remnants do not have to be deleted immediately, but they should not be cited as active method descriptions.
...
4. and it layers exploratory clustering plus biological annotation on top.
...
- and clearer retirement or isolation of experimental clustering branches.
