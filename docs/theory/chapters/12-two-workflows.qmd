...
| **Flag** | `--stability` then `--freeze` | `--predictor-only` |
| **Steps run per iteration** | centroid → detector → classifier → predictor | predictor only |
| **Steps run once (at freeze)** | centroid → detector(fixed) → classifier → mapper → enricher | — |
| **Output** | production classifier + full gene report | `all_metrics.csv`, `metrics_summary.json` |
...
MethylValidation addresses this by repeating the entire pipeline — from centroid construction through prediction — on random train/validation splits. Each iteration detects DMPs on the training cohort only, builds a classifier from those DMPs, and evaluates it on the held-out validation cohort. The resulting balanced accuracy estimates are honest internal measures of generalization.
...
3. `methyl-classifier --project` builds binary OvR classifiers.
4. `methyl-predictor --project` evaluates on the validation split and writes `validation_metrics.json`.
...
This calls `freeze_production_model()` which:
...
3. Runs the full production pipeline on **all samples**: centroid → detector(fixed panel) → classifier → mapper → enricher.
4. Writes `monte_carlo_runs/production/production_summary.json`.
...
\node[box, below=of B] (C) {Per iteration: centroid $\to$ detector $\to$ classifier $\to$ predictor};
\node[box, below=of C] (D) {\texttt{validation\_metrics.json} per iteration};
...
\node[box, below=of I] (J) {Full pipeline on \emph{all} samples (no predictor)};
\node[box, below=of J] (K) {Final classifier + mapper + enricher outputs};
...
└── production/
    ...
    ├── classifiers/
    │   └── multiclass-classifier.pkl   ← the final production model
    ├── mappings/
    └── enrichments/
```
