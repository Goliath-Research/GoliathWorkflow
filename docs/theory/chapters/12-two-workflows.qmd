...
For the held-out claim to be valid, the frozen model must not have been trained on the hold-out samples. When `holdout_exclude_from_training=true` (default), the `--freeze` step filters the hold-out samples out of the production cohort list files before building centroids/detector/classifier, and records the excluded set in `monte_carlo_runs/production/holdout_manifest.json`. Workflow 3 preflights that manifest and refuses to run if any current hold-out sample was not excluded, so a false held-out claim cannot be produced silently.
