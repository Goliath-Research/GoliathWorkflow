...
**Practical role.** This stage answers: "What trained classifier and predictor artifacts should be deployed?" In practice it:

- runs `methyl-classifier` on `production/project.json`,
- runs `methyl-predictor` to generate production-side metrics/artifacts,
- enforces the optional biological gate (`require_biological_review_for_model`) before model creation.
