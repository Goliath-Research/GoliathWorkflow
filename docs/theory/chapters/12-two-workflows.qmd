...
flowchart TD
    ...
    MC["methyl-validation --project … --stability\n(n_iterations × full pipeline)"]
    SplitIter["Per iteration:\ncentroid → detector → classifier → predictor"]
    Metrics["validation_metrics.json\n(per iteration)"]
    Stability["Stability analysis:\ncompute f(d) across iterations"]
    StablePanel["stability/stable_dmps_production.csv\n(DMPs with f(d) >= stability_dmp_freq)"]
    Freeze["methyl-validation --project … --freeze"]
    Merge["Merge stable DMPs\nstable_dmps_genomewide.csv"]
    ProdProject["production/project.json\n(with fixed_dmp_panel)"]
    FullPipeline["Full pipeline on ALL samples:\ncentroid → detector(fixed) → classifier → mapper → enricher"]
    FinalModel["production/ directory\nFinal classifier + gene report"]

    ProjectConfig --> MC
    MC --> SplitIter
    SplitIter --> Metrics
    Metrics --> Stability
    Stability --> StablePanel
    StablePanel --> Freeze
    Freeze --> Merge
    Merge --> ProdProject
    ProdProject --> FullPipeline
    FullPipeline --> FinalModel
```
...
```{mermaid}
flowchart TD
    FrozenProject["production/project.json\n(frozen production model)"]
    ProjectConfig["project.json\n(defines cohorts + validation settings)"]
    PO["methyl-validation --project … --predictor-only\n(n_iterations × predictor only)"]
    SplitIter["Per iteration:\nstratified split → merge frozen paths → methyl-predictor"]
    Metrics["validation_metrics.json\n(per iteration)"]
