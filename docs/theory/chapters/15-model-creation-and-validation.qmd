...
Two simplex sources use this uniformly:

- **First-stage ECDF class probabilities.** They sum to one, so they are always
  ALR-encoded. For the binary case ($K=2$) the single coordinate
  $\log\frac{p_1+\varepsilon}{p_0+\varepsilon}$ is the clipped class-1 log-odds;
  raw dual probabilities are never stacked.
- **Cell-type fractions and other declared groups.** Operators declare each
  ...
  reference defaulting to the last column, pseudocount, and whether the ALR
  coordinates are standardized). Parts must be disjoint from each other and from
  numeric/ordinal/categorical roles, so raw proportions never enter the design
  matrix. Deconvolution outputs (Houseman or HiTIMED, see @sec-methyldeconv) are
  the primary example.

ALR coordinates live on $\mathbb{R}$, so standardizing them (the default) is
meaningful; probabilities and their ALR coordinate are left on their natural
scale.
