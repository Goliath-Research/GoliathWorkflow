...
This layer is controlled by `multiclass_train_learned_head` and related hyperparameters in `step_config.detection` (see @sec-configuration-reference). It is especially useful when the raw histogram scores have good ranking behavior but imperfect class boundaries.

### Isotonic and Stacking Layers

Two additional modeling switches in `step_config.classifier` are important for current production behavior:
