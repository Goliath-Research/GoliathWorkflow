...
| `grok_batch_size` | int | `20` | Gene batch size per Grok API request. |
...
| `libraries` | list[str] | `["KEGG_2021_Human", "Reactome_2022", "GO_Biological_Process_2023", "MSigDB_Hallmark_2020"]` | Enrichr library names for over-representation analysis. |
...
## `step_config.cluster` {#sec-ref-cluster}

*See @sec-methylcluster for theoretical background.*

| Parameter | Type | Description |
|---|---|---|
| `clustering_method` | str | Algorithm: `"hdbscan"`, `"hierarchical"`, `"kmeans"`, `"gmm"`. |
| `metric` | str | Distance metric: `"jensenshannon"`, `"euclidean"`, etc. |
| `min_cluster_size` | int | HDBSCAN minimum cluster size. |
| `min_samples` | int | HDBSCAN minimum samples. |
| `cluster_selection_epsilon` | float | HDBSCAN epsilon for flat extraction. |
| `cluster_selection_method` | str | `"eom"` or `"leaf"`. |
| `allow_single_cluster` | bool | Allow HDBSCAN to return a single cluster. |
| `linkage_method` | str | Hierarchical linkage: `"ward"`, `"complete"`, `"average"`. |
| `max_k` | int | Maximum $k$ for automatic $k$ selection (hierarchical/KMeans). |
| `force_k` | int \| null | Force a specific $k$ (overrides auto-selection). |
| `silhouette_threshold` | float | Minimum silhouette score for valid clusters. |
| `enable_medoid_refinement` | bool | Refine cluster assignments using medoids. |
| `use_gpu` | bool | Use GPU for distance matrix computation. |
