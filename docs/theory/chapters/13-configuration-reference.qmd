...
| Parameter | Type | Default | Constraints | Description |
|---|---|---|---|---|
| `min_coverage` | int | `4` | ≥ 1 | Minimum total (mC + uC) reads per position per sample. Positions below this threshold are excluded from the centroid. **Should match `step_config.detection.min_coverage`.** |
...
| `feature_family_set` | str | `"dmp_scored"` | `dmp_scored`, `gene`, `structural`, `gene_scored`, `dmp_scored+gene`, `dmp_scored+structural`, `dmp_scored+gene_scored`, `hybrid-all` (legacy aliases `dmp`, `dmp+gene`, `dmp+structural`, `dmp+gene_scored` accepted) | Feature-family contract for observed-hybrid model bundles and predictors. Applies only when `feature_mode=observed_hybrid`. |
