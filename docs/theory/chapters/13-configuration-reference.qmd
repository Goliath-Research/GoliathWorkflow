...
| `model_weight_column` | str | `"weight"` | — | Preferred detector DMP column used as feature weight in model bundle construction; falls back to `effect_size` or `1.0`. |
| `tabular_model_type` | str | `"random_forest"` | `"random_forest"`, `"hist_gradient_boosting"`, `"logistic_regression"` | Model type when `model_backend="tabular_sklearn"`. |
| `tabular_max_dmps` | int | `5000` | ≥ 10 | Maximum number of DMP loci retained from bundle index for tabular/generative backend training. |
...
|---|---|---|
| `step_config.validation.run_mapper_and_enricher` | Legacy behavior flag | Deprecate, then remove. Freeze stage owns mapper/enricher execution. |
| `step_config.validation.skip_enricher` | Legacy companion flag | Deprecate, then remove. Use explicit freeze/enricher step policy instead. |
| `step_config.validator` | Deprecated alias of `step_config.predictor` | Remove after one full release cycle of warnings. |
...
#### 1) `step_config.validator` -> `step_config.predictor`
...
#### 2) Remove `run_mapper_and_enricher` / `skip_enricher` from `step_config.validation`
...
After rewriting config keys, rerun:
