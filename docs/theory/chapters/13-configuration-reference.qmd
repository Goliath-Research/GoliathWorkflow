...
| `feature_family_set` | str | `"dmp"` | `dmp`, `gene`, `structural`, `dmp+gene`, `dmp+structural`, `hybrid-all` | Feature-family contract for observed-hybrid model bundles and predictors. |
| `ecdf_aggregated_enabled` | bool \| null | `null` | — | Toggle aggregated ECDF OvR for `model_backend=ecdf`. `null` auto-enables when `feature_mode=observed_hybrid` and `feature_family_set!=dmp`. |
