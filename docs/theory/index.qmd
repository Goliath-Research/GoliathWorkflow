...
| @sec-methyldeconv | `methyldeconv` | Houseman flat and HiTIMED hierarchical cell-type deconvolution (analyte-driven trees) |
...
**Part I: Mathematical and Statistical Foundations** covers the algorithms and estimators in order from the lowest shared layer to the downstream applications. Readers interested in the production classifier should start with @sec-methylutils, then continue through @sec-methylcentroid, @sec-methyldetector, and @sec-methylclassifier. Readers interested in biological interpretation should continue to @sec-methylmapper, @sec-methyldeconv, and @sec-methylenricher. @sec-limitations is intentionally blunt and should be read before any publication claim is finalized.

**Part II: Configuration Semantics and Workflow Theory** documents study manifest anatomy (@sec-project-configuration), the two operational workflow models (@sec-two-workflows), and model creation/validation theory (@sec-model-creation-validation).

**Operator commands and deployment** live in the [Usage manual](../usage/index.qmd). **Parameter lookup** lives in [Reference: configuration-reference](../reference/configuration-reference.qmd). **Implementation internals** live in [Implementation guide](../implementation/index.md).

### Package Map

| Part | Chapter | Primary packages | Role |
|---|---|---|---|
| I | @sec-methylutils | `methylutils` | shared mathematical layer |
| I | @sec-methylcentroid | `methylcentroid` | centroid construction |
| I | @sec-methyldetector | `methyldetector` | DMP testing and panel export |
| I | @sec-methylclassifier | `methylclassifier` | classification heads |
| I | @sec-methylpredictor-validation | `methylpredictor`, `methylvalidation`, `methyldiseaseprogression` | validation and freeze theory |
| I | @sec-methylmapper | `methylmapper` | DMP-to-gene mapping |
| I | @sec-methyldeconv | `methyldeconv` | Houseman / HiTIMED cell deconvolution |
| I | @sec-methylenricher | `methylenricher` | enrichment and modules |
| I | @sec-methylalignmentqc | `methylalignmentqc` | QC parsing |
| I | @sec-limitations | all | caveats and legacy |
| II | @sec-project-configuration | `methylutils` (ProjectConfig) | study manifest semantics |
| II | @sec-two-workflows | all | workflow theory |
| II | @sec-model-creation-validation | `methylvalidation` | model lifecycle theory |
