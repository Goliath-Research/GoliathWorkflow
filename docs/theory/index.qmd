...
| @sec-methyldetector | `methyldetector` | DMP testing, filtering, classifier export, and production panel bypass |
| @sec-methylclassifier | `methylclassifier` | binary, multi-chromosome, OvR, and multiclass learned-head classification |
| @sec-methylpredictor-validation | `methylpredictor`, `methylvalidation`, `methyldiseaseprogression` | prediction metrics, Monte Carlo validation, stability analysis, production freeze, progression synthesis |
...
**Part I: Mathematical and Statistical Foundations** covers the algorithms and estimators in order from the lowest shared layer to the downstream applications. Readers interested in the production classifier should start with @sec-methylutils, then continue through @sec-methylcentroid, @sec-methyldetector, and @sec-methylclassifier. Readers interested in biological interpretation should continue to @sec-methylmapper and @sec-methylenricher. @sec-limitations is intentionally blunt and should be read before any publication claim is finalized.

**Part II: Workflows and Production** describes the two end-to-end operational workflows supported by MethylPipeline: Model Creation (repeated validation, stability analysis, production freeze, optional progression synthesis, and final `--model` step) and Model Use for Prediction (predictor-only evaluation of a frozen model). It also documents the project configuration file as the single source of truth for the entire pipeline.

**Part III: Configuration Reference** provides exhaustive tables of every parameter in every `step_config` section, with types, defaults, constraints, and descriptions. It is a reference, not a tutorial.

**Part IV: User Guide** provides step-by-step commands, output directory layout, result interpretation, and a diagnostics section covering the three most common failure modes observed in practice: low DMP coverage, class imbalance, and model version mismatch.

### Package Map (updated)

| Part | Chapter | Primary packages | Role |
|---|---|---|---|
| I | @sec-methylutils | `methylutils` | shared mathematical layer: centroids, ECDF views, testing, effect sizes, classifier densities |
| I | @sec-methylcentroid | `methylcentroid` | centroid construction and coverage preprocessing |
| I | @sec-methyldetector | `methyldetector` | DMP testing, filtering, classifier export, and production panel bypass |
| I | @sec-methylclassifier | `methylclassifier` | binary, multi-chromosome, OvR, and multiclass learned-head classification |
| I | @sec-methylpredictor-validation | `methylpredictor`, `methylvalidation` | prediction metrics, Monte Carlo validation, stability analysis, production freeze |
| I | @sec-methylcluster | `methylcluster` | beta-based and distance-based clustering workflows |
| I | @sec-methylmapper | `methylmapper` | DMP-to-gene mapping and gene-level aggregation |
| I | @sec-methylenricher | `methylenricher` | over-representation analysis, pathway graph clustering, and module scoring |
| I | @sec-methylalignmentqc | `methylalignmentqc` | deterministic QC parsing and summarization |
| I | @sec-limitations | all | caveats, legacy remnants, and claims that should be softened |
| II | @sec-project-configuration | `methylutils` (ProjectConfig) | anatomy of the project JSON as the single source of truth |
| II | @sec-two-workflows | all | the two operational workflows with theory, commands, and diagnostics |
| III | @sec-configuration-reference | all | exhaustive step_config parameter tables |
| IV | @sec-user-guide | all | end-to-end user guide |
