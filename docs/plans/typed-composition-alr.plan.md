...
> **Status: IMPLEMENTED.** `CompositionGroup` model + `covariate_composition_groups` on the validation backend params; the covariate preprocessor simplex-encodes each group (raw multi-part proportions never enter X). **Binary encode updated by** [`ecdf-h5-binary-p.plan.md`](ecdf-h5-binary-p.plan.md): \(K=2\) → closed non-reference \(p\); ALR only for \(K>2\). Legacy `covariate_composition_*` and `ecdf_second_stage_probability_transform: logit_class1` resolve to the encode path. Profiles `cell_deconv`, `cell_deconv_hitimed`, and `samd_research` migrated; schemas regenerated; regression tests in `packages/methylvalidation/tests/test_composition_groups.py` and updated `test_ecdf_second_stage_covariates.py`.
