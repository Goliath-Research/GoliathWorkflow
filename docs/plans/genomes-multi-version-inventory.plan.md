...
overview: Company myQNAPcloud holds a multi-version genome inventory under epimethyl/genomes/ using a role-based tree (linear / annotation / pangenome). Site/cfg pins exact selected versions; provision syncs from epimethyl-genomes to /work/genomes.

> **Status: IMPLEMENTED.** Canonical tree + site `reference_selection`, `epimethyl-genomes` endpoint, reference_asset fixtures/SQL seeds, `s3_sync` provision, Phase 0 `scripts/provision_selected_genomes.sh`.
...
todos:
  ...
  - id: qnap-reorganize-cli
    content: QNAP epimethyl/genomes/ reorganized to canonical tree (operator complete)
    status: completed
  - id: confirm-gap-docs
    content: Document Phase 0 sync from epimethyl/genomes/ inventory + site pins selected versions
    status: completed
  - id: seed-genomes-endpoint
    content: Seed/publish cfg storage_endpoint epimethyl-genomes (bucket epimethyl, prefixBase genomes/) reusing archive credential
    status: completed
...
```
epimethyl/genomes/   ↔   /work/genomes/
...
| Endpoint seed | [`portal_resource_profile.sql`](../../workflow_engine/sql_mssql/portal_resource_profile.sql) → `epimethyl-genomes` |
