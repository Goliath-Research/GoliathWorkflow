...
- Extend [docs/research/methylpipeline_informme_integration.md](../research/methylpipeline_informme_integration.md) with a short "v2 equilibrium Ising (implemented) vs dynamic capacity/RDE (deferred)" section and the GPU-batching approach.
- Add a v2 note to [docs/reference/read_level_pattern_contract.md](../reference/read_level_pattern_contract.md) clarifying the contract is unchanged (per-tile `k`), and to the promoted plan [docs/plans/read-level-info-measures.plan.md](../plans/read-level-info-measures.plan.md) referencing the follow-on.
