...
todos:
  ...
  - id: trainer-gate-all-modes
    content: Run second stage for raw_dmp/raw_gene/aggregated when ecdf_second_stage_enabled or covariates_path set
    status: completed
...
- [`trainer_api.py`](../../packages/methylvalidation/methyl_validation/trainer_api.py): gate on `ecdf_second_stage_enabled` **or** `covariates_path` for raw_dmp, raw_gene, and aggregated paths
