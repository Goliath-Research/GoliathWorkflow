...
| [`effective-monte-carlo-config.plan.md`](effective-monte-carlo-config.plan.md) | _(pending ADO)_ | Effective Monte Carlo configuration | `legacy-inventory-removal`, `canonical-schemas-configs`, `effective-schema-writer`, `breaking-change-verification` |
