...
| [`prostate-options-catalog.plan.md`](prostate-options-catalog.plan.md) | _(pending ADO)_ | Prostate cancer researcher options catalog | `draft-ch25`, `index-links`, `promote-plan` |
| [`mhl-survival-alternative.plan.md`](mhl-survival-alternative.plan.md) | _(pending ADO)_ | Native MHB/MHL + survival alternative | `bam-tags`, `mhap-extract`, `mhb-mhl-action`, `panel-qc`, `survival-backend`, `procedure-program` |
...
| [`layered-guardrails-editor.plan.md`](layered-guardrails-editor.plan.md) | _(pending ADO)_ | Layered SamplePrep guardrail editors | `types-schemas`, `seed-data-types`, `sql-twins`, `tests`, `deploy-mcp`, `docs-ia` |
...
| [`deploy-remote-gateway.plan.md`](deploy-remote-gateway.plan.md) | _(ops)_ | Deploy gateway.goliathresearch.com + worker cutover | `inventory-remote`, `install-gateway`, `nginx-tls`, `verify-health`, `worker-cutover`, `rotate-creds` |
