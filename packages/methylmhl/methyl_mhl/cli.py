...
from .core.runner import run_mhb_mhl_for_project
from .project_resolver import resolve_mhb_mhl_step_config


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Discover MHBs and compute per-sample MHL from haplotype sidecars.",
    )
    ...
    cfg, samples, out_dir = resolve_mhb_mhl_step_config(
        args.project,
        args.step_override,
    ...
    if summary.get("status") == "skipped":
        print("[WARN] Skipped: no haplotype sidecars found.")
    else:
        print(
            f"[INFO] Wrote MHL matrix for {summary.get('n_samples', 0)} "
            f"sample(s), {summary.get('n_blocks', 0)} block(s) to {out_dir}"
        )
