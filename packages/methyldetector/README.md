...
methyl-detector /configs/healthy_vs_cancer.json
...
1. **`methyl_detector_classifier.pkl`** - Trained model for MethylClassifier
2. **`methyl_detector_selected_dmps.csv`** - Final DMP set used in classifier
3. **`methyl_detector_all_dmps.csv`** - All detected DMPs (pre-binary search)
4. **`methyl_detector_summary.json`** - Complete analysis metadata

### Additional Outputs

5. **`methyl_detector_gene_weights.csv`** - Gene-level feature importance
...
📚 **[Comprehensive Documentation](docs/METHYLDETECTOR_COMPREHENSIVE_DOCUMENTATION.md)** - Complete guide covering:
...
# Load summary
with open('/output/detector/methyl_detector_summary.json') as f:
    summary = json.load(f)
...
# Load DMP table
dmps = pd.read_csv('/output/detector/methyl_detector_selected_dmps.csv')
print(dmps.head())

# Load gene weights
genes = pd.read_csv('/output/detector/methyl_detector_gene_weights.csv')
...
MethylDetector uses **Balanced Accuracy** instead of AUC for DMP selection because:
...
See [Classification Documentation](docs/Classification.md) for detailed comparison with AUC.
...
```python
run_comparison(config: MethylDetectorConfig) -> Dict[str, Any]
```
...
See [Comprehensive Documentation](docs/METHYLDETECTOR_COMPREHENSIVE_DOCUMENTATION.md) for complete API reference.
...
@software{methyldetector2024,
  title={MethylDetector: DMP Detection and Bayesian Classifier Training},
...
- [MethylDetector Comprehensive Documentation](docs/METHYLDETECTOR_COMPREHENSIVE_DOCUMENTATION.md)
