...
Optional **filter funnel exploration** (`filter_funnel_explore`): sweep `effect_size_coverage` over a min/max/step range in one run and write `filter_funnel-{chromosome}.csv` (columns: `n_statistical_dmps`, `effect_size_coverage`, `n_biological_dmps`) for charting; see README “Filter funnel exploration”.
