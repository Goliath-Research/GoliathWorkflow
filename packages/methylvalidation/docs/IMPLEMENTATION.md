...
| **stability/dmp_frequency_chr_<chrom>.html** | Per-chromosome Plotly charts with `all` vs `selected` count curves and elbow marker (max distance-to-baseline) when curve geometry supports detection. |
