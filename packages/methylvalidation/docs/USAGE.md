...
- **ECDF class probabilities:** the ECDF second-stage stacker treats the first-stage `prob_class*` columns as a built-in composition group and always ALR-encodes them (binary gives one coordinate equal to the clipped class-1 logit; `ecdf_second_stage_probability_epsilon` sets the pseudocount). Raw dual probabilities are no longer stacked; `ecdf_second_stage_probability_transform: "logit_class1"` is accepted as an alias for this default.
