...
- **Composition (simplex) covariates:** any feature set that sums to one (cell fractions, and other proportion vectors) must be declared as a **composition group** so it is encoded by additive log-ratio (ALR) instead of fed as raw, collinear proportions. This applies to **all** backends that consume covariates (`tabular_sklearn`, `generative_hybrid`, and the ECDF second stage). Set `covariate_composition_groups` to a list of `{name, columns, pseudocount, reference?, standardize?}`; `pseudocount` is required (profile/site; no code default), `reference` defaults to the last column, `standardize` defaults to true. ALR drops the reference and emits `K - 1` coordinates named `alr_<part>_vs_<reference>`. Composition parts must be disjoint from each other and from `covariate_numeric_columns` / `_ordinal_columns` / `_categorical_columns`. The legacy single-group keys (`covariate_composition_transform`/`_columns`/`_reference`/`_pseudocount`) still resolve to one group named `default` but are deprecated.
...
- **ECDF class probabilities:** the ECDF second-stage stacker treats the first-stage `prob_class*` columns as a built-in composition group and always ALR-encodes them (binary gives one coordinate equal to the clipped class-1 logit). Operators must set `ecdf_second_stage_probability_epsilon` (ALR pseudocount) in profile/site actionConfig whenever the stacker runs; there is no code default. Raw dual probabilities are no longer stacked; `ecdf_second_stage_probability_transform: "logit_class1"` is accepted as an alias for the ALR path.
